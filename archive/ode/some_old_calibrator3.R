...
#--------------------------------- PLOT COUNTY CURVES  -----------------------
ggplot(nat_cases %>% filter(lk == "Bremen"), aes(x = time, y = active_case_prop, color = lk)) +
  geom_line() +
  theme(legend.position = "none",
        axis.title = element_text(size = 18, face = "bold"),
        axis.text = element_text(size = 16))

ggplot(nat_cases, aes(x = time, y = active_case_prop, color = lk)) +
  geom_line() +
  theme(legend.position = "none",
        axis.title = element_text(size = 18, face = "bold"),
        axis.text = element_text(size = 16))
...
library(lhs)
b <- randomLHS(1, 398)
b2 <- matrix(nrow = nrow(b), ncol = ncol(b))
b2[1,] <- qunif(b[1,], min = (1/14), max = (6/14))
# values to start optimisation
beta_start  <- b2[1,]
names(beta_start) <- paste("beta", rownames(nat_mobility$movement_prob_matrix), sep = "_")
# alpha_nat_start <- nat_mobility$alpha
m <- randomLHS(1, 398)
m2 <- matrix(nrow = nrow(m), ncol = ncol(m))
m2[1,] <- qnorm(m[1,], mean = nat_mobility$alpha, sd = 0.1)
alpha_nat_start <- m2[1,]
...
# likelihood function
sir_ll <- function(parameters, dat, params_fit) {
  # times - dense timesteps for a more detailed solution
  times <- seq(from = 0, to = days, by = 1)
  
  parms <- list(beta = plogis(unname(parameters[1:398])), alpha_nat = plogis(unname(parameters[399:796])))
  # print(parms$beta)
...
# TODO need to double check if beta is actually calculated correctly here

mle_fit <- optim(par = c(qlogis(beta_start), qlogis(alpha_nat_start)),
                 fn = sir_ll,
                 dat = nat_cases %>% select(time, j_county, active_case_prop),
                 params_fit = params_fixed,
                 method = "Nelder-Mead",
                 control = list(maxit = 1E5, trace = 2))

save(mle_fit, file = paste("results/",
                           "mle",
                           "beta",
                           start_date,
                           end_date,
...
#---------------------------------------
# TODO check if these params make sense and of alpha_nat is still similar (maybe try with only beta as free param)
# TEST FITTED VALUES
params_test <- list(
  beta = plogis(mle_fit$par[1:398]),
  alpha_nat = plogis(mle_fit$par[399:796]),
  gamma = 1 / 14, # average recovery rate [d^-1]  §
