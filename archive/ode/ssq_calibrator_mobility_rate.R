...
SIR_SSQ <- function(parameters, dat, params_fit) {  # parameters must contain beta & alpha
  ...
  parms <- list(beta = unname(parameters[1:398]), alpha_nat = unname(parameters[399:length(parameters)]))
  times <- seq(from = 0, to = days, by = 0.5)
  
  # print(c(parms, params_fit))
  ...
  result <- as.data.frame(ode(y = initial_state_values  # vector of initial state 
                              ...
                              , parms = c(parms, params_fit)       # the parameters argument
                            )
...
ssq <- optim(par = c(beta = beta_start, alpha_nat_start),
               fn = SIR_SSQ,
...
ssq <- optim(par = c(beta_start, alpha_nat_start),
                 fn = sir_ll,
                 dat = nat_cases %>% select(time, j_county, active_case_prop),
                 params_fit = params_fixed,
                 method = "Nelder-Mead",
