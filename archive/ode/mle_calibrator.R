...
sir_ll <- function(lbeta, lalpha_nat, lalpha_int) {
  ...
  parms <- c(beta = plogis(lbeta), alpha_nat = plogis(lalpha_nat), alpha_int = plogis(lalpha_int))
...
fit_mle <- mle2(sir_ll,
              start = list(lbeta = qlogis(beta_start), lalpha_nat = qlogis(alpha_nat_start), lalpha_int = qlogis(alpha_int_start)),
              method = "Nelder-Mead",
