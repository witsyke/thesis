...
sir_ll <- function(params) {
  ...
  parms <- list(beta = plogis(params[1]), alpha_nat = plogis(params[2]), alpha_int = plogis(params[3]))
...
parnames(sir_ll) <- c("lbeta", "lalpha_nat", "lalpha_int")

# minimize negative-log-likelihood
fit_mle <- mle2(sir_ll,
              start = c(lbeta = qlogis(beta_start), lalpha_nat = qlogis(alpha_nat_start), lalpha_int = qlogis(alpha_int_start)),
              method = "Nelder-Mead",
